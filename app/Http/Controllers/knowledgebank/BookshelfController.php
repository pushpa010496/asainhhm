...
class BookshelfController extends HomeController
{
	...
	{	
		 $data = BookShelf::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('bookshelf_image','url','title_tag','alt_tag','title','id','no_pages','publisher','publisher_date','authors','description','created_at','updated_at')->first();
		 if(empty($data)){
