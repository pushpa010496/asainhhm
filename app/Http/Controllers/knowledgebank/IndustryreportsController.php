...
class IndustryreportsController extends HomeController
{
	...
	{	
		$industryreports = IndustryReport::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('title2','title1','full_report','id','short_description','created_at','updated_at')->first();
		// dd($industryreports);
	...
	public function more($offset){
		...
		$data = IndustryReport::where('active_flag',1)->orderBy('id','desc') ->offset($offset)->limit(10)->select('url','title1','title_tag','short_description')->get();
