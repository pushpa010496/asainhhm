...
class InterviewsController extends HomeController
{
	...
	{	
		$interviews = Interview::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('photo','title_tag','alt_tag','title','designation','name','description','quest_answer','id','created_at','updated_at')->first();
         if(empty($interviews)){
