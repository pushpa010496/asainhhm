...
class PressreleaseController extends HomeController
{
	...
	{	
		...
		$pressreleases = Pressrelease::select('url','title','date','short_description')->where('active_flag',1)->orderBy('id','desc')->limit(10)->get();;	
	...
	{	
		$pressreleases = Pressrelease::select('title','date','description','id','short_description','created_at','updated_at')->where('url',$url)->where('active_flag',1)->orderByDesc('id')->firstOrFail();
	...
	public function more($offset){
		...
		$data = Pressrelease::where('active_flag',1)->orderBy('id','desc')->offset($offset)->select('url','title','date','short_description')->limit(10)->get();
	 ...
	 public function filter($month,$year){
	 	$pressreleases = 	Pressrelease::where('active_flag', 1)->whereRaw("MONTH(date) = ?", [$month])->whereRaw("YEAR(date) = ?", [$year])->orderByDesc('id')->limit(10)->get();
	 	$data_count = Pressrelease::select('url','title','date','short_description')->where('active_flag',1)->whereRaw("MONTH(date) = ?", [$month])->whereRaw("YEAR(date) = ?", [$year])->orderBy('id','desc')->count(); 
	 	$banners = $this->banners;
     ...
     public function listpressreleasesFilter($month,$year,$offset){
         ...
         $data =Pressrelease::where('active_flag',1)->whereRaw("MONTH(date) = ?", [$month])->whereRaw("YEAR(date) = ?", [$year])->orderBy('id','desc')->offset($offset)->limit(10)->get(); 
