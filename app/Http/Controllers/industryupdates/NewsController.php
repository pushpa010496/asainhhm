...
class NewsController extends HomeController
{
	...
	{	
        ...
        $news = News::select('url','title','date','home_description')->where('active_flag',1)->orderBy('id','desc')->limit(20)->get();
        $banners = $this->banners;
        $data_count = News::select('url','title','date')->where('active_flag',1)->count(); 
    ...
    {	        
      ...
      $news = News::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('title','date','description','id','short_description','created_at','updated_at')->first();
      if(empty($news)){
  ...
  public function more($offset){
    ...
    $data = News::where('active_flag',1)->orderBy('id','desc')->offset($offset)->limit(10)->select('url','title','date','home_description')->get();
...
public function filter($month,$year){
 ...
 $news = News::where('active_flag',1)->whereIn(DB::raw("Month(date)"), [$month])->whereIn(DB::raw("Year(date)"), [$year])->orderBy('id','desc')->select('id','url','title','date','home_description')->distinct()->get()->take(10);
 $data_count = News::where('active_flag',1)->whereIn(DB::raw("Month(date)"), [$month])->whereIn(DB::raw("Year(date)"), [$year])->orderBy('id','desc')->select('url','title','date','home_description')->distinct()->count(); 
 $banners = $this->banners;
...
public function listnewsFilter($month,$year,$offset){
   $data =News::where('active_flag',1)->whereIn(DB::raw("Month(date)"), [$month])->whereIn(DB::raw("Year(date)"), [$year])->orderBy('id','desc')->offset($offset)->limit(10)->select('url','title','date','home_description')->get(); 
