...
public function healthcaremanagement()
{
    ...
    $banners = $this->banners;
    $healthcarectg=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','1')->orderBy('id', 'desc')->get()->take(20);
   
   $data_count = Editorialarticle::where('category_id','1')->where('active_flag',1)->count();
...
{
  $data=Editorialarticle::with('authors','category')->where('active_flag',1)->where('category_id','1')->where('url',$slug)->orderBy('id', 'desc')->first(); 
  if(empty($data)){
 ...
 $current_issue =  $this->current_issue;
  ...
  if($current_issue->id == $data->issue->id){
     ...
     {
     return redirect('login')->with('url',$slug)->with('category',$data->category->url)->with('latestarticle','true');
     }
  }
 return view('editorial.view',compact('data','banners'));
}
...
{
    $healthcarectg=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','2')->orderBy('id', 'desc')->get()->take(20);
    $banners = $this->banners;
    $current_issue =  $this->current_issue;
...
public function  medicalsciencesview($slug)
{
   ...
   $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','2')->where('url',$slug)->where('category_id',2)->first();
   if(empty($data)){
    return view('errors.404');
  }        
   $banners = $this->banners;
  ...
  //  $key_value=  Session::get('key');
    ...
    //   return view('editorial.view',compact('data','banners'));
    // }
 //   return redirect()->route('medical-sciences.slug',['slug' => $slug])->with('url',$slug)->with('category',$data->category->url)->with('status','true');
...
{
    $healthcarectg=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','3')->orderBy('id', 'desc')->get()->take(20);
...
public function surgicalview($slug)
{
   ...
   $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','3')->where('url',$slug)->where('active_flag', 1)->first();    
 if(empty($data)){
    return view('errors.404');
  }     
   $banners = $this->banners;
...
//       return view('editorial.view',compact('data','banners'));
//     }
//     return redirect('login')->with('url',$slug)->with('category',$data->category->url)->with('status','true');
//    // return view('auth.login');
//   }
   return view('editorial.view',compact('data','banners'));
}
...
{
    $healthcarectg=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','4')->orderBy('id', 'desc')->get()->take(20);
    $banners = $this->banners;
    $current_issue =  $this->current_issue;
...
public function diagnosticsview($slug)
{
   ...
   $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','4')->where('url',$slug)->first();  
   if(empty($data)){
    return view('errors.404');
  }       
   $banners = $this->banners;
...
{
    $healthcarectg=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','5')->orderBy('id', 'desc')->get()->take(20);
    $banners = $this->banners;
    $current_issue =  $this->current_issue;
...
public function technologyview($slug)
{
    ...
    $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','5')->where('url',$slug)->first();  
    if(empty($data)){
    return view('errors.404');
  }       
    $banners = $this->banners;
...
public function facilitiesview($slug)
{
    ...
    $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','6')->where('url',$slug)->first(); 
...
public function informationview($slug)
{
 ...
 $data=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','7')->where('url',$slug)->first();     
 if(empty($data)){
    return view('errors.404');
  }    
 $banners = $this->banners;
...
{
   $data=Editorialarticle::with('category')->where('category_id','9')->where('url',$slug)->first();    
   if(empty($data)){
    return view('errors.404');
  }     
   $banners = $this->banners;
...
public function forewordview($slug,Request $request)
{

    $data=Editorialarticle::with('category')->where('category_id','8')->where('url',$slug)->first();  
