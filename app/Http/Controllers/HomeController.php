...
$bookshelfs = BookShelf::where('active_flag',1)->select('bookshelf_image','url','title_tag','alt_tag','title','authors','description','no_pages')->inRandomOrder()->take(1)->get();

       $articles = Article::with('authors')->where('home_articles',1)->where('active_flag', 1)->orderBy('id','desc')->take(4)->get();
     //  $articles = Article::with('authors')->where('home_articles',1)->where('active_flag', 1)->inRandomOrder()->take(4)->get();
// dd($articles);
      // $articles=DB::SELECT('CALL Get_articles()');
       ...
       // $casestudy = CaseStudy::where('active_flag',1)->where('home_casestudies', 1)->orderBy('id', 'desc')->select('url','title','short_description','title_tag')->first();
        ...
        $advaisoryboard = AdvaisoryBoard::where('active_flag', 1)->orderBy('id', 'desc')->take(12)->select('image','alt_tag','title_tag','title','home_description')->get();
...
//dd($advaisoryboard);
       ...
       $healthcarectgs=Editorialarticle::with('category')->where('active_flag',1)->where('category_id','1')->orderBy('id', 'desc')->get()->take(4);
       ...
       // $diagnostics = Editorialarticle::where('category_id', 4)->where('active_flag', 1)->where('on_home', 1)->orderBy('id','desc')->select('url','title_tag','title','short_description')->first();
        ...
        return view('home/index' ,compact('pages','banners','news','bookshelfs','pressreleases','articles','healthcarectgs','interviews','whitepaper','events','reaserchinsites','casestudy','technotrend','sliders','news','advaisoryboard','techno_equipment','surgical_speciality','information_technology','medical_science','diagnostics','facilities','ebook'));
    }  
