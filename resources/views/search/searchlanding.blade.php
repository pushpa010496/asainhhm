...
<link rel="stylesheet" href="https://industry.asianhhm.com/mix/app.css">
 <script src="https://industry.asianhhm.com/mix/app.js"></script>

<style type="text/css">

  .TXT-BLUE{color:#4b4849 !important;font-size: 18px;padding-bottom:7px;}
  .TXT-GREEN{color:#6a6a6a !important;font-size: 14px;font-weight:bold;padding-bottom:4px;}
...
@section('content')
    ...
    <div class="container PF-BrdrDDD mt-4 pl-5 mb-5">
      <div class="PF-BrdrBEEE col-lg-12 col-md-12 col-sm-12 col-xs-12 p-0 mb-3">
        <h1 class="PF-Bshelf pt-2" style="font-size:30px;">SEARCH RESULT</h1>            
      </div>
        ...
        <div class="tab-content mt-3" id="myTabContent">
          ...
          <div class="tab-pane fade show active" id="tab_all" role="tabpanel" aria-labelledby="home-tab">
            @if(sizeof($earticles) != 0)
              @foreach($earticles as $val)              
                ...
                <p class="TXT-GREEN mb-1"><a href="{{ url($val['url']) }}" class="TXT-GREEN"><i>{{ url($val['url']) }}</i></a></p>
            ...
            @foreach($authors as $val)
                ...
                <h2 class="TXT-BLUE mb-1 author" style="color:#000 !important;">
          ...
          @if(sizeof($knowledgebank) != 0)            
              @foreach($knowledgebank as $val)
                <h2 class="TXT-BLUE mb-1" style="color:#000 !important;">
                  <a href="{{ url($val->url) }}" class="TXT-BLUE">{{ $val->title }}</a>
                </h2>
                <p class="TXT-GREEN mb-1"><a href="{{ url($val->url) }}" class="TXT-GREEN"><i>{{ url($val->url) }}</i></a></p>
                <p class="TXT-BLK">{{ \Illuminate\Support\Str::limit(preg_replace('/[^a-zA-Z\s]/', '', strip_tags(html_entity_decode(ucfirst(@$val->description)))),$limit = 400, $end = '...') }}
          ...
          @if(sizeof($authors) != 0)
              <div class="tab-pane fade show" id="tab-authors" role="tabpanel" aria-labelledby="home-tab">
                @foreach($authors as $val)
                  <h2 class="TXT-BLUE mb-1" style="color:#000 !important;">
                    <a href="{{ url($val['url']) }}" class="TXT-BLUE"><i>{{ $val['title'] }}</i></a>
                  </h2>
                  <p class="TXT-GREEN mb-1"><a href="{{ url($val['url']) }}" class="TXT-GREEN"><i>{{ url($val['url']) }}</i></a></p>
                  <p class="TXT-BLK">{{ \Illuminate\Support\Str::limit(preg_replace('/[^a-zA-Z\s]/', '', strip_tags(html_entity_decode(ucfirst(@$val['description'])))),$limit = 400, $end = '...') }}
                  </p>
